...
fetch('/api/status',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('Status unavailable');return r.json();}).then(data=>{apiReady=Boolean(data.ready);text('connection',apiReady?'AI lab ready':'API setup needed');$('connection').dataset.ready=String(apiReady);text('tutor-status',apiReady?'Make a prediction and run the lab to explore together.':'Built-in lesson available · AI features need API setup.');}).catch(()=>text('connection','Server unavailable'));
